...
describe('Web UI 401 redirect logic in index.ejs', () => {
  ...
  it('guards auto-redirect with hadRefreshToken check', () => {
    ...
    const pattern = /if \(res\.status === 401\) \{\s*\/\/ Only auto-redirect to Monzo auth if a refresh token existed[\s\S]*?if \(hadRefreshToken\) \{\s*window\.location\.href = '\/auth';[\s\S]*?\}\s*return;/;
